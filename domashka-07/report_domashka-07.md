...
Застосунок: **лічильник візитів** — Flask + PostgreSQL, ~40 рядків Python.
Кожен GET `/` додає рядок у таблицю і повертає номер візиту, час запису
(його проставляє сам Postgres — колонка `ts timestamptz DEFAULT now()`)
і скільки секунд минуло з попереднього візиту.
...
| Пункт | Результат |
|---|---|
| Образ | `python:3.12-slim`, 5 шарів, 173 МБ; exec-форма CMD; контекст збірки — 1.50 кБ завдяки `.dockerignore` |
| Кеш | 1-ша збірка **5.61 с** (з них pip install 4.3 с) → 2-га після зміни одного символу **0.84 с** |
...
- **`.dockerignore`** — виключає `__pycache__`, `.venv`, `.git`, md-файли
  й сам Dockerfile. Контекст збірки — **1.50 кБ** (лише `app.py` і
...
28.3 МБ залежностей + 1.4 кБ коду (`docs/image-info.txt`).
...
|---|---|---|---|
| 1-ша | — | **5.61 с** | усі шари з нуля (`--no-cache`); сам `pip install` — 4.3 с |
| 2-га | 1 символ у `app.py` | **0.84 с** | `WORKDIR`, `COPY requirements.txt`, `RUN pip install` — **CACHED**; перевиконано лише `COPY app.py` |
...
(1.4 кБ, 0.0 с). Якби код і залежності копіювались одним `COPY . .` до
...
$ curl localhost:8007/   # двічі, з паузою 3 с
Привіт!! Це візит номер 28.
Час візиту: 2026-07-30 21:10:27 UTC.
З попереднього візиту минуло 42.1 с.
Привіт!! Це візит номер 29.
Час візиту: 2026-07-30 21:10:30 UTC.
З попереднього візиту минуло 3.0 с.
```

Том працює: після `docker compose down && up` (контейнери знищено і
створено заново) лічильник продовжив з того ж місця — `візит номер 30,
минуло 14.4 с`. Час візитів проставляє сам Postgres (`DEFAULT now()`)
і зберігає в томі, тому інтервали коректно рахуються навіть через
перестворення контейнерів — одного разу застосунок чесно показав
інтервал у ~267 тисяч секунд (три доби між сесіями роботи).

![Стек domashka-07 у Docker Desktop](docs/docker-desktop-stack.png)

Стек `domashka-07` у списку контейнерів Docker Desktop: працює,
CPU ~0.02% — лічильник візитів у стані спокою хост не навантажує.
...
1. **Порядок інструкцій у Dockerfile — це і є кеш-стратегія.** Стабільне
   (залежності) — вище, мінливе (код) — нижче: збірка з 5.61 с
   перетворюється на 0.84 с, і різниця росте з розміром залежностей.
