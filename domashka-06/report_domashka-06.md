...
> кроки (dig, route, traceroute, дамп, curl -w, openssl) прогнано повторно,
> скріншоти перезнято. Локальна мережа тепер 192.168.8.0/24 зі шлюзом
...
$ traceroute -I -n -w 2 -m 20 ithillel.ua
 1  192.168.8.1     2.2 ms   ← мій (новий) роутер
 2  10.20.48.1      4.9 ms   ← приватна мережа провайдера
 3  10.12.18.1      4.4 ms
 4  10.1.1.3        5.3 ms
...
**Що це означає:** до сайту тепер — **13 хопів** (було 12). Порівняння з
попереднім прогоном пояснює різницю: перший хоп змінився з 10.20.50.1 на
192.168.8.1, а другим з'явився 10.20.48.1 — сусідній блок тієї самої
10.20.48.0/22-мережі провайдера, куди раніше входила і моя стара підмережа
10.20.50.0/24. Тобто «роутером» 10.20.50.1 у ДЗ 4 було обладнання
провайдера, а тепер перед ним стоїть мій власний роутер зі своїм
NAT-сегментом 192.168.8.0/24 — звідси й додатковий хоп. Далі шлях
ідентичний: приватні адреси провайдера, публічна IPORT, IX, Hetzner.
...
**MSS 1400** — хтось на шляху (найімовірніше, новий роутер або тунель
провайдера) робить MSS clamping під менший MTU, і сегменти даних тепер
по 1388 байтів корисного навантаження замість 1448.
...
Мій «продакшн» — домашня мережа на роутері GL.iNet (щойно замінив ним
Nest Wifi; у дампі його видно за OUI вендора GL Technologies) плюс
