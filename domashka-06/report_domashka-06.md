...
Мій «продакшн» — два VPS (Linode і Hetzner) за **Traefik**, а вдома —
Synology з Docker-стеком і Mac mini з власними сервісами за каскадом
GL.iNet + Nest Wifi (новий роутер у дампі видно за OUI вендора
GL Technologies). Ось що я б змінив після цього розбору.

1. У Traefik зафіксував би явні `tls.options` — мінімум TLS 1.2 і
   cipher-список без CBC (з генератора Mozilla): рівно та розбіжність,
   яку знайшов у розділі 8 в ithillel.ua; дефолти Traefik пристойні, але
   явна конфігурація не пливе при оновленнях.
2. Увімкнув би HSTS через headers-middleware Traefik
   (`max-age=63072000; includeSubDomains`, після перевірки субдоменів —
   preload), бо без нього перший заход користувача по http:// лишається
   вікном для downgrade-атаки.
3. Traefik сам продовжує Let's Encrypt-сертифікати, але моніторинг усе
   одно повісив би (`openssl x509 -checkend` у cron з алертом за
   14 днів): 70 днів з розділу 6 швидко стають нулем, а зламане
   авто-продовження у Traefik — типово тихий фейл, видний лише в логах.
4. На домашньому роутері вимкнув би відповіді на ICMP/закриті порти з WAN
   не чіпаючи ICMP типу 3/4 (fragmentation needed) — інакше зламається
   Path MTU Discovery, який розбирали в ДЗ 4; MSS clamping з розділу 4 —
   живе нагадування, що MTU вдома тепер нестандартний.
5. Доступ до домашніх сервісів (Docker на Synology, Mac mini) тримав би
   через WireGuard-тунель до одного з VPS, а не через проброс портів —
   тим більше що вдома тепер подвійний NAT (GL.iNet за Nest Wifi), крізь
   який port forwarding і так не працює без налаштування обох роутерів.
