...
Власного публічного сервісу в мене немає — мій «продакшн» це домашня
мережа, де за Nest Wifi тепер каскадом стоїть роутер GL.iNet (у дампі
його видно за OUI вендора GL Technologies). Тому пункти 1–3 — на випадок,
якби я щось публікував, а 4–5 — про мережу, яка в мене реально є.

1. Якби піднімав сайт за nginx — одразу вставив би рядок `ssl_ciphers`
   з генератора Mozilla, без CBC-шифрів у TLS 1.2, — рівно та
   розбіжність, яку знайшов у розділі 8 в ithillel.ua.
...
3. Повісив би моніторинг терміну сертифіката (`openssl x509 -checkend` у
   cron з алертом за 14 днів): 70 днів з розділу 6 швидко стають нулем,
